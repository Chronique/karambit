...
;; initialize tanpa owner check - siapapun bisa init (hanya sekali)
(define-public (initialize (target-maturity-block uint))
  (begin
    ...
    (try! (contract-call? .kbt-pt3 set-maturity-block target-maturity-block))
    (try! (contract-call? .kbt-yt3 set-maturity-block target-maturity-block))
    (ok true)))
...
(define-public (mint-pt-yt (sy-amount uint))
  (let ((caller tx-sender))
    ...
    (try! (contract-call? .kbt-pt3 mint sy-amount caller))
    (try! (contract-call? .kbt-yt3 mint sy-amount caller))
...
(define-public (redeem-pt (pt-amount uint))
  (let ((caller tx-sender))
    ...
    (try! (contract-call? .kbt-pt3 burn pt-amount caller))
...
(define-public (redeem-early (amount uint))
  (let ((caller tx-sender))
    ...
    (try! (contract-call? .kbt-pt3 burn amount caller))
    (try! (contract-call? .kbt-yt3 burn amount caller))
    (try! (as-contract (contract-call? .kbt-sy transfer
...
(define-public (claim-yield)
  (let ((caller tx-sender)
        (claimable (contract-call? .kbt-yt3 get-pending-yield caller)))
    (asserts! (> claimable u0) ERR-ZERO-AMOUNT)
    (try! (contract-call? .kbt-yt3 reset-pending-yield caller))
...
(define-public (collect-and-distribute-yield (new-yield-amount uint))
  (let ((total-yt (unwrap-panic (contract-call? .kbt-yt3 get-total-supply)))
        (current-index (contract-call? .kbt-yt3 get-yield-index)))
    ...
    (let ((index-delta (/ (* new-yield-amount PRECISION) total-yt)))
      (try! (contract-call? .kbt-yt3 update-yield-index
